...
class MsProductCompositeApplicationTests {
	...
	void setUp() {
		...
		when(recommendationIntegration.getRecommendations(PRODUCT_ID_OK)).thenReturn(
				singletonList(new RecommendationDto(PRODUCT_ID_OK, 1, "author", 1, "content", "mock address")));
		when(reviewIntegration.getReviews(PRODUCT_ID_OK)).thenReturn(
	...
	void getProductById() {
		...
		client.get().uri("/v1/product-composite/" + PRODUCT_ID_OK).accept(APPLICATION_JSON).exchange().expectStatus()
	...
	void getProductNotFound() {
		...
		client.get().uri("/v1/product-composite/" + PRODUCT_ID_NOT_FOUND).accept(APPLICATION_JSON).exchange()
				.expectStatus().isNotFound().expectHeader().contentType(APPLICATION_JSON).expectBody()
				.jsonPath("$.path").isEqualTo("/v1/product-composite/" + PRODUCT_ID_NOT_FOUND).jsonPath("$.message")
	...
	void getProductInvalidInput() {
		...
		client.get().uri("/v1/product-composite/" + PRODUCT_ID_INVALID).accept(APPLICATION_JSON).exchange().expectStatus()
				.isEqualTo(UNPROCESSABLE_ENTITY).expectHeader().contentType(APPLICATION_JSON).expectBody()
				.jsonPath("$.path").isEqualTo("/v1/product-composite/" + PRODUCT_ID_INVALID).jsonPath("$.message")
